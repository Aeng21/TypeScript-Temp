...
async function loadData() {
    try {
        ...
        if (res.ok && result.success) renderData(result.data);
...
function renderData(data: any[]) {
    ...
    data.forEach((item, index) => {
        ...
        const nama = escapeHtml(String(item.nama));
        const alamat = escapeHtml(String(item.alamat));
        ...
        html += `
          <tr>
            ...
            <td>${rank}</td>
            <td>
              <button class="action-btn edit-btn" data-id="${id}" data-nama="${nama}" data-alamat="${alamat}" data-rank="${rank}">Edit</button>
              <button class="action-btn delete-btn" data-id="${id}">Hapus</button>
            </td>
          </tr>
        `;
    });
