...
`controllers/playerController.ts` — project ini sudah pakai validasi **Zod** (`playerSchema`), bukan lagi `if (!nama || !alamat || !rank)` manual yang menerima `hobi` apa adanya (`hobi ?? []` cuma jaga-jaga kalau `undefined`, tidak memvalidasi ISI arraynya sama sekali — request curl/Postman bisa saja mengirim `hobi: ["<script>", "apapun"]`). Karena pilihannya tetap (sesuai HTML: Gaming/Membaca/Olahraga), pakai `z.array(z.enum(...))` supaya isinya juga divalidasi, bukan cuma tipe array-nya:
```ts
...
```
`.default([])` membuat field ini boleh tidak dikirim sama sekali (dianggap array kosong), tapi kalau dikirim, tiap elemennya WAJIB salah satu dari 3 pilihan itu — bukan string bebas.

Di `create()` dan `update()`, tinggal tambahkan `hobi` ke destructuring `parsed.data`, ke pemanggilan `PlayerModel.create`/`update`, dan ke response `data` (tidak perlu `hobi ?? []` manual lagi, `.default([])` di skema sudah menanganinya):
```ts
const { nama, alamat, rank, hobi } = parsed.data;                                                                        // <- BAGIAN INI
