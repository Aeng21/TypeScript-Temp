...
`controllers/playerController.ts` — project ini sudah pakai validasi **Zod** (`playerSchema`), bukan lagi `if (!nama || !alamat || !rank)` manual yang tidak mengecek `isActive` sama sekali. Tambahkan ke skema yang sudah ada:
```ts
...
```
Karena `isActive` sekarang divalidasi Zod sebagai boolean sungguhan, konversi manual `!!isActive` yang lama sudah tidak diperlukan lagi — `parsed.data.isActive` sudah pasti `boolean`, bukan nilai lain yang cuma "dipaksa" jadi boolean.

Di `create()` dan `update()`, tinggal tambahkan `isActive` ke destructuring `parsed.data`, ke pemanggilan `PlayerModel.create`/`update`, dan ke response `data`:
```ts
const { nama, alamat, rank, isActive } = parsed.data;                                                                        // <- BAGIAN INI
