...
// Mengimpor helmet: middleware yang memasang beberapa HTTP response header
// terkait keamanan sekaligus (contoh: mencegah browser menebak-nebak
// Content-Type/"MIME sniffing", menonaktifkan header X-Powered-By, dsb).
// Tanpa ini, header keamanan tersebut tidak ada sama sekali secara default.
import helmet from 'helmet';

// Mengimpor express-rate-limit: middleware untuk membatasi jumlah request
// dari satu alamat IP dalam rentang waktu tertentu. Berguna untuk mencegah
// brute-force atau spam request (misalnya bot yang memanggil endpoint
// berkali-kali per detik) membebani server/database.
...
// helmet() dipasang PALING AWAL (sebelum middleware lain) supaya header
// keamanannya berlaku untuk semua response, termasuk file statis dan error.
app.use(helmet());
...
//
// SEBELUMNYA: cors() dipanggil TANPA opsi sama sekali, artinya SEMUA origin
// (situs mana pun di internet) diizinkan memanggil API ini dan membaca hasilnya.
// Itu berbahaya untuk API yang (di masa depan) menyimpan data sensitif, dan
// tetap merupakan praktik buruk meski datanya publik, karena situs lain bisa
// menyalahgunakan API kita dari browser korban tanpa sepengetahuan mereka.
// SEKARANG: origin dibatasi hanya ke FRONTEND_URL (alamat frontend kita
// sendiri), dengan fallback ke http://localhost:2000 (port dev frontend)
// kalau environment variable FRONTEND_URL belum diset.
...
// { limit: '10kb' } membatasi ukuran maksimal body request. Tanpa batas ini,
// seseorang bisa mengirim body raksasa (misal beberapa ratus MB) berulang-ulang
// untuk menghabiskan memori/bandwidth server (salah satu bentuk serangan
// Denial of Service). 10kb sudah lebih dari cukup untuk data player (nama,
// alamat, rank, masing-masing maksimal 100 karakter).
...
// Rate limiter umum: membatasi setiap alamat IP maksimal 100 request dalam
// jendela waktu 15 menit KHUSUS untuk endpoint /api/player. Kalau terlampaui,
// express-rate-limit otomatis membalas dengan status 429 (Too Many Requests).
// Ini mencegah satu klien (sengaja atau tidak, misal bug infinite-loop di
// frontend, atau bot) memanggil API berkali-kali per detik sampai membebani
// koneksi pool database.
const playerRateLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 menit, dalam milidetik.
  limit: 100,                // maksimal 100 request per IP dalam jendela waktu di atas.
  standardHeaders: true,     // kirim info limit lewat header standar RateLimit-*.
  legacyHeaders: false,      // matikan header lama X-RateLimit-* (tidak perlu dua-duanya).
});
...
// playerRateLimiter dipasang SEBELUM playerRoutes supaya berlaku untuk semua
// method (GET/POST/PUT/DELETE) di bawah '/api/player'.
app.use('/api/player', playerRateLimiter, playerRoutes);
...
// p_err bertipe 'unknown' (bukan 'any' lagi) karena Express tidak menjamin
// bentuk objek error yang dilempar oleh middleware/route sebelumnya.
app.use((p_err: unknown, p_req: Request, p_res: Response, p_next: NextFunction) => {
  // Selalu catat detail LENGKAP error di log server untuk kebutuhan debugging,
  // TIDAK PERNAH dikirim mentah-mentah ke client.
  console.error(p_err);
  p_res.status(500).json({
    success: false,
    message: 'Terjadi kesalahan pada server',
    // Pesan error asli hanya disertakan ke response saat NODE_ENV BUKAN
    // 'production', supaya detail internal server tidak bocor ke publik saat
    // aplikasi benar-benar live/production.
    ...(process.env.NODE_ENV !== 'production' && {
      error: p_err instanceof Error ? p_err.message : 'Unknown error',
    }),
  });
});
