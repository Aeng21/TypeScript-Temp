...
class UserModel {
  ...
  static async create(p_username: string, p_password: string, p_role: UserRole): Promise<number> {
    ...
    // Angka 12 = "cost factor" (jumlah putaran hashing berlipat dua tiap +1 angka), makin besar
    // makin aman (makin lama & mahal di-brute-force oleh penyerang) tapi makin lambat diproses.
    // Dinaikkan dari 10 -> 12 mengikuti rekomendasi umum saat ini (2020-an ke atas) untuk
    // mengimbangi hardware penyerang yang makin cepat; +2 masih cukup ringan untuk 1 login/register,
    // tapi jauh lebih mahal untuk penyerang yang mencoba brute-force jutaan kombinasi.
    const hashed = await bcrypt.hash(p_password, 12);
